...
try:
    for densidadActualSim in range(densidadMin,densidadMax+1,2): #Densidad (#targets/celda)
        ...
        for j in range(7): #Sim hasta:(a) todos los UAV llegaron, o, (b) targets agotados
            for i in range(50):
                print("Actualización: "+str(i)+", en iteracion: "+ str(j))
                ...
                for celdaActual in range(1,div**2+1): #recorrido sobre todas las celdas
                    ...
                    if aNumber!=0:
                        ...
                        if asignados>0:
                            ...
                            distancias=np.array(CurrentCellInfo[aNumber:aNumber+asignados])-np.array(CurrentCellInfo[0:asignados])[:,[0,1]]
                            ...
                            ids=ids[:,2]
                            print(ids)
                            print(CurrentCellInfo[aNumber:aNumber+asignados]*arrivingBoolMatrix)
                        ...
                        if asignados<aNumber:
                            ...
                            for asignando in range(aNumber-asignados):
                                if(deltaMatrix[2*currentRow,currentColumn]-asignadosBlw>0): #delta blw
                                    vunit=np.array([0,-1,0]) #abajo
                                    asignadosBlw=asignadosBlw+1
                                elif not (currentRow == div):
                                    vunit=np.array([0,1,0]) #arriba
                                elif (currentRow==div):
                                    if(deltaRight[currentColumn]-asignadosRght>0): #delta rght
                                        vunit=np.array([1,0,0])
                                        asignadosRght=asignadosRght+1
                                    elif not(currentColumn==1):
                                        vunit=np.array([-1,0,0])
                                dx=vunit*dt
                                print("aNumber is: ", aNumber, ", asignados is: ", asignados, "and asignando is: ", asignando)
                                CurrentCellInfo[asignados+asignando]=CurrentCellInfo[asignados+asignando]+dx
    ...
    print("deltaT is: ",i,", iter is: ", j, "aNumber is: ",aNumber)
